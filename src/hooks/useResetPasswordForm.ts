...
export function useResetPasswordForm(handleReset: (password: string) => void) {
    ...
    const handlePasswordBlur = () => {
        ...
        if (!validation.isValid) setPasswordError(validation.error || 'Hasło musi mieć minimum 6 znaków');
    ...
    const handleSubmit = () => {
        ...
        if (!v1.isValid || !v2.isValid || password !== password2) {
            setPasswordError(v1.error || 'Hasła muszą być identyczne i mieć minimum 6 znaków');
            return;
        }
