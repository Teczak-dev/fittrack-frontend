...
export const useAddWorkout = (switchDisplay:()=>void) => {
    ...
    const handleMinutes = (value: string) => {
        ...
        if (!validateMinutes(v)) setMinutesError('Czas trwania musi być większy niż 0');
        setMinutes(v);
    };
    ...
    const handleCalories = (value: string) => {
        ...
        const v = parseInt(value || '0', 10) || 0;
        if (!validateCalories(v)) setCaloriesError('Liczba spalonych kalorii musi być większa niż 0');
        setCalories(v);
    };
    ...
    const handleAddClick = async (e: any) => {
        ...
        const newWorkout: Workout = {
            ...
            duration: minutes,
            calories: calories,
            category: workoutCategory.find( (item) => item.name === (workoutName || workoutCategory[0]?.name) )?.category || 'Inne',
