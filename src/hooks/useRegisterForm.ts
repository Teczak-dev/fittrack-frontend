...
// Define the function type for registration
type RegisterFn = (email: string, pass: string, username: string) => void;

// Custom hook to manage the registration form state and validation
// It accepts a registration function as a parameter
// and returns the form state, field setters, blur handlers, submit handler, and reset function.
export const useRegisterForm = (onRegister: RegisterFn) => {
  ...
  const handleEmailBlur = () => {
    ...
    if (!emailValidation.isValid) {
      dispatch({
        type: "SET_ERROR",
        field: "email",
        error: emailValidation.error || "Podaj poprawny email",
      });
    } else {
  ...
  const handlePasswordBlur = () => {
    ...
    if (!passwordValidation.isValid) {
      dispatch({
        type: "SET_ERROR",
        field: "password",
        error: passwordValidation.error || "Hasło musi mieć minimum 6 znaków",
      });
    } else {
  ...
  const handleConfirmBlur = () => {
    ...
    if (!confirmValidation.isValid) {
      dispatch({
        type: "SET_ERROR",
        field: "confirm",
        error: confirmValidation.error || "Hasła nie są identyczne",
      });
    } else {
  ...
  const handleUsernameBlur = () => {
    ...
    if (!usernameValidation.isValid) {
      dispatch({
        type: "SET_ERROR",
        field: "username",
        error: usernameValidation.error || "Nazwa musi mieć minimum 3 znaki",
      });
    } else {
  ...
  const handleSubmit = () => {
    ...
    if (!passwordValidation.isValid) {
      errors.password =
        passwordValidation.error || "Hasło musi mieć minimum 6 znaków";
      hasErrors = true;
    }
    if (!usernameValidation.isValid) {
      errors.username =
        usernameValidation.error || "Nazwa musi mieć minimum 3 znaki";
      hasErrors = true;
    }
    if (!confirmValidation.isValid) {
